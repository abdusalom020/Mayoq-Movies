...
let foundMovies = movies.slice(0, 50);
...
function onMoviesListInfoButtonClick(evt) {
  ...
  if (evt.target.matches('.js-bookmark-button')) {
    ...
    const movie = movies.find(movie => movie.imdbId === elBookmarkButton.dataset.imdbId);
    const movieIndexInWatchlist = watchlist.findIndex(movie => movie.imdbId === elBookmarkButton.dataset.imdbId);

    if (movieIndexInWatchlist === -1) {
      watchlist.push(movie);

      elBookmarkButton.textContent = 'Bookmarked ✅';
      elBookmarkButton.classList.remove('btn-outline-secondary');
      elBookmarkButton.classList.add('btn-secondary');
    } else {
      watchlist.splice(movieIndexInWatchlist, 1);

      elBookmarkButton.textContent = 'Bookmark';
      elBookmarkButton.classList.remove('btn-secondary');
      elBookmarkButton.classList.add('btn-outline-secondary');
    }
